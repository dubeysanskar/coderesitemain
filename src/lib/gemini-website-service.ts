...
export class GeminiWebsiteService {
  ...
  async generateWebsite(description: string): Promise<{ html: string; instructions: string }> {
    const websitePrompt = `
      Create a fully functional, accessible web application based on this description: "${description}"

      CRITICAL ACCESSIBILITY REQUIREMENTS (WCAG 2.1 AA):
      ...
      - Include embedded CSS and JavaScript within the HTML
      ...
      The website should be complete, production-ready, and fully accessible.
    `;
    ...
    const instructionsPrompt = `
      ...
      2. Key accessibility features (3-4 bullet points)
      3. Tips for best experience (1-2 helpful tips)
